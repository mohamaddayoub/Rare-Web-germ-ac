...
<head>
    ...
    {{-- No Online Consultation Css File Because I Used Tailwind Css --}}
...
<body>
    ...
    <section class="cover cover-4">
        <div class="content">
            <h1>{{ __('translate.Online Consultation') }}</h1>
            <ol>
                <li>
                ...
                <li>
                    {{ __('translate.Online Consultation') }}
                </li>
            </ol>
        </div>
    </section>
    ...
    <div class="my-container bg-king text-white">
        <h1
            ...
            {{ __('translate.online-consultation-head') }}</h1>
        <p class="text-lg md:text-xl text-center mb-5 md:mb-10">
            {{ __('translate.online-consultation-text') }}
        </p>

        @if ($sections !== null)
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 md:gap-3 lg:gap-5 text-center pt-5">
                @foreach ($sections as $section)
                    <form action="{{ route('sections/doctors') }}" method="post" role="form">
                        @csrf
                        <div
                            ...
                            <button type="submit" class="content cursor-pointer text-sm md:text-lg lg:text-xl">
                                {{-- {{ $section->name }} --}}
                                {{-- {{ __('translate.section-' . $section->id) }} --}}
                                {{-- {{ GoogleTranslate::trans($section->name, app()->getLocale()) }} --}}

                                @if ( __('translate.section-' . $section->id) !== 'translate.section-' . $section->id)
                                    {{ __('translate.section-' . $section->id) }}
                                @else
                                    {{ GoogleTranslate::trans($section->name, app()->getLocale()) }}
                                @endif
